...
func registerResultArtifacts(server *mcp.Server, kali *kaliclient.Client) error {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Read any retained artifact completely through bounded pages. Use offset and limit for raw byte paging, or start_line and line_count for UTF-8 line paging; the modes are mutually exclusive. Select raw for every artifact or stdout/stderr for tool-result-json. A line over 64 KiB returns line_truncated and continues through next_offset. No cumulative read limit is imposed. Check encoding and redaction_state before handling content.",
		InputSchema: schema,
