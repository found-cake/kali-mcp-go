...
8. For a tool expected to exceed the MCP host deadline, call run_tool_async with the dedicated tool name and its arguments, retain the returned job_id, and retrieve it with scan_job_status or scan_job_result. Async preserves arguments.timeout; after a process timeout, use a longer timeout or narrower scope in a new run. After process exit, terminal job lookup remains available for the requested execution timeout plus a three-minute grace period, so repeated polling is unnecessary; scan_job_cancel stops pending work. Async jobs are process controls, not durable workflow state or scanner-native resume checkpoints.
