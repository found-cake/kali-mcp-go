...
var scanToolCapabilities = []dto.ScanToolCapability{
	...
	toolCapability("nuclei_scan", "nuclei", "/api/tools/nuclei/stream", "Run template-based vulnerability checks with Nuclei. DoS, fuzz, and interactsh templates are excluded by default; omitting tags and templates evaluates all locally installed safe templates and emits a scope warning.", dto.TargetInputURLOrHost, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSafeRecon}, rateAndConcurrencyControls(false)),
