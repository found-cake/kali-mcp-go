...
var scanToolCapabilities = []dto.ScanToolCapability{
	...
	toolCapability("feroxbuster_scan", "feroxbuster", "/api/tools/feroxbuster/stream", "Recursively discover web content with Feroxbuster and automatic tuning.", dto.TargetInputWebURL, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSafeRecon, dto.ProfileWebDiscoveryLowRate}, rateAndConcurrencyControls(false)),
	toolCapability("nuclei_scan", "nuclei", "/api/tools/nuclei/stream", "Run template-based vulnerability checks with Nuclei. DoS, fuzz, and interactsh templates are excluded by default.", dto.TargetInputURLOrHost, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSafeRecon}, rateAndConcurrencyControls(false)),
	...
	toolCapability("jwt_analyze", "jwt_tool", "/api/tools/jwt/stream", "Decode and assess JWTs for alg=none, forced-error, playbook, or key-confusion cases with jwt_tool.", dto.TargetInputToken, dto.ImpactActive, dto.ToolExecutionStream, false, nil, nil),
	...
	toolCapability("browser_check", "browser-check", "/api/tools/browser/stream", "Verify DOM-XSS execution at full SPA or hash-route URLs and inspect Chromium dialogs, console, network, screenshot, and rendered DOM evidence.", dto.TargetInputWebURL, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileBrowserXSSConfirm}, nil),
	toolCapability("retirejs_scan", "retire", "/api/tools/retire/stream", "Scan a page URL, explicit public JavaScript bundles, or a local bundle path with Retire.js.", dto.TargetInputURLOrFile, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSafeRecon}, nil),
	...
	toolCapability("http_request", "http-request", "/api/tools/http-request", "Send one bounded HTTP request to an explicitly selected target for manual validation.", dto.TargetInputWebURL, dto.ImpactActive, dto.ToolExecutionPost, false, []dto.SafetyProfile{dto.ProfileSafeRecon}, nil),
}
