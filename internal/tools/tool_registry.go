...
var scanToolCapabilities = []dto.ScanToolCapability{
	...
	toolCapability("nuclei_scan", "nuclei", "/api/tools/nuclei/stream", "Run template-based vulnerability checks with Nuclei. Safe-recon execution requires severity, tags, or templates; dry_run may preview all local safe templates without contacting the target. DoS, fuzz, DAST, OAST, and interactsh templates are excluded by default.", dto.TargetInputURLOrHost, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSafeRecon}, append(rateAndConcurrencyControls(false), dto.ScanControlCapability{Control: dto.ScanControlDryRun, Enforcement: dto.ControlServerPreview})),
