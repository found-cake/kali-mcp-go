...
var scanToolCapabilities = []dto.ScanToolCapability{
	...
	toolCapability("metasploit_run", "msfconsole", "/api/tools/metasploit", "Run a specified Metasploit module against the authorized target.", dto.TargetInputModule, dto.ImpactExploit, dto.ToolExecutionPost, false, []dto.SafetyProfile{}, nil),
	toolCapability("hydra_attack", "hydra", "/api/tools/hydra", "Run a short credential audit with Hydra. Use hydra_attack_stream for file-based or long-running attempts.", dto.TargetInputNetworkHost, dto.ImpactCredential, dto.ToolExecutionPost, false, []dto.SafetyProfile{}, nativeControls(dto.ScanControlConcurrency)),
	...
	toolCapability("dirb_scan", "dirb", "/api/tools/dirb/stream", "Discover web paths and content with Dirb and a wordlist. Tested-word progress is suppressed and discovered paths are returned as structured results. Dirb has no enforceable request-rate control; use FFUF or Feroxbuster when a low-rate profile is required.", dto.TargetInputWebURL, dto.ImpactActive, dto.ToolExecutionStream, true, []dto.SafetyProfile{dto.ProfileSafeRecon}, nil),
	toolCapability("nikto_scan", "nikto", "/api/tools/nikto/stream", "Check a web server for common misconfigurations and known vulnerability patterns with Nikto. The caller selects installed plugins exposed by get_scan_capabilities. web-discovery-low-rate enforces a native request pause and an internal completion deadline; put_del_test requires explicit-custom because it changes target state.", dto.TargetInputURLOrHost, dto.ImpactActive, dto.ToolExecutionStream, true, []dto.SafetyProfile{dto.ProfileWebDiscoveryLowRate}, nil),
	toolCapability("sqlmap_scan", "sqlmap", "/api/tools/sqlmap/stream", "Verify a SQL-injection hypothesis from a URL, POST body, or raw HTTP request with SQLmap. Typed fields expose SQLmap's native true/false response oracles, payload prefix and suffix, and test-title filter without accepting arbitrary payload engines. Use raw_request for HTTP methods other than the URL/Data flow. sqli-verify-low-risk pins risk 1, level 1, and techniques BEU while rejecting method overrides, takeover, writes, and broad extraction.", dto.TargetInputURLOrFile, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSQLILowRisk}, rateAndConcurrencyControls(false)),
	...
	toolCapability("feroxbuster_scan", "feroxbuster", "/api/tools/feroxbuster/stream", "Recursively discover web content with Feroxbuster and automatic SPA fallback calibration. Response events are deduplicated and returned through discovered_paths while raw JSONL remains in the artifact. Authenticated coverage requires caller-supplied request context.", dto.TargetInputWebURL, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSafeRecon, dto.ProfileWebDiscoveryLowRate}, rateAndConcurrencyControls(false)),
	toolCapability("nuclei_scan", "nuclei", "/api/tools/nuclei/stream", "Run template-based vulnerability checks with Nuclei. dry_run reports the local selected template count without target traffic; normal scans do not run a duration-estimation preflight. Inline output contains only complete bounded lines; page the raw stdout artifact by line for full JSONL. Nuclei's native rate limiter is an average throttle that may burst above the requested value in a rolling one-second window. Safe-recon requires a selector and excludes DoS, fuzz, DAST, OAST, and interactsh templates; a completed safe scan is not exhaustive coverage.", dto.TargetInputURLOrHost, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSafeRecon}, append(nucleiControls(), dto.ScanControlCapability{Control: dto.ScanControlDryRun, Enforcement: dto.ControlServerPreview})),
	...
	toolCapability("browser_check", "browser-check", "/api/tools/browser/stream", "Verify DOM-XSS execution at full SPA or hash-route URLs and inspect Chromium dialogs, console, network, screenshot, and rendered DOM evidence. Per-call headers, cookies, and local_storage are isolated to the selected origin and discarded with the browser context.", dto.TargetInputWebURL, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileBrowserXSSConfirm}, nil),
	...
	toolCapability("http_request", "http-request", "/api/tools/http-request", "Send one bounded HTTP request to an explicitly selected target for manual validation. virtual_host may change only the HTTP Host value when target_context fixes the connection address. Raw request and response evidence is preserved unless redact_values is explicitly supplied; safe-recon permits only GET, HEAD, and OPTIONS.", dto.TargetInputWebURL, dto.ImpactActive, dto.ToolExecutionPost, false, []dto.SafetyProfile{dto.ProfileSafeRecon}, nil),
}
...
func rateControls(observe5xx bool) []dto.ScanControlCapability {
	return appendObservedControls(nativeControls(dto.ScanControlRateLimit), observe5xx)
}

func rateAndConcurrencyControls(observe5xx bool) []dto.ScanControlCapability {
	return appendObservedControls(nativeControls(dto.ScanControlRateLimit, dto.ScanControlConcurrency), observe5xx)
}

func nucleiControls() []dto.ScanControlCapability {
	return []dto.ScanControlCapability{
...
func appendObservedControls(controls []dto.ScanControlCapability, observe5xx bool) []dto.ScanControlCapability {
	if observe5xx {
