...
var scanToolCapabilities = []dto.ScanToolCapability{
	...
	toolCapability("sqlmap_scan", "sqlmap", "/api/tools/sqlmap/stream", "Verify a SQL-injection hypothesis from a URL, JSON body, or raw HTTP request with SQLmap. sqli-verify-low-risk pins conservative detection settings and rejects takeover, write, and broad extraction options.", dto.TargetInputURLOrFile, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{dto.ProfileSQLILowRisk}, rateAndConcurrencyControls(false)),
	...
	toolCapability("jwt_analyze", "jwt_tool", "/api/tools/jwt/stream", "Parse JWT structure and metadata offline, with optional live endpoint verification for alg=none, forced-error, playbook, or key-confusion cases using jwt_tool.", dto.TargetInputToken, dto.ImpactActive, dto.ToolExecutionStream, false, []dto.SafetyProfile{}, nil),
