...
The container uses Docker's default network. Scan tools never rewrite loopback targets: `127.0.0.1`, `localhost`, and `[::1]` always refer to the runtime where `kali-server` is running. Call `resolve_target` first to inspect the Kali runtime, `host.docker.internal` when it resolves, and the Linux default gateway. Each candidate includes a `browser_target` URL when the input was a URL, a `network_target` hostname or IP for tools such as Nmap, and the probed port. The resolver performs only bounded TCP connects and returns tool-specific recommendations only when exactly one candidate is reachable. Select the appropriate form explicitly and pass the same resolution receipt to the scan. On Linux, persistent containers may need `--add-host host.docker.internal:host-gateway`.
