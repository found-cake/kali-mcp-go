...
Dedicated scan requests accept a `profile` plus optional `timeout_request_budget`, `rate_limit`, `concurrency`, `health_url`, and `max_5xx_responses` controls. `timeout_request_budget` is an estimate used with `rate_limit` to derive an outer timeout; it is not a hard request limit. Available profiles are:
