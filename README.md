...
| Scan controls | Profiles, explicit timeouts, rate limits, concurrency limits, health checks, and bounded 5xx circuit breaking |
...
- `execution`: redacted argv, tool version, start/end time, timeout, profile, rate, concurrency, health URL, and 5xx threshold
...
Dedicated scan requests accept a `profile` plus optional `timeout`, `rate_limit`, `concurrency`, `health_url`, and `max_5xx_responses` controls when supported by that tool. The MCP server executes the selected scope and reports actual duration and observed request counts; it does not predict scan duration or derive timeout values from estimated request counts. Available profiles are:
